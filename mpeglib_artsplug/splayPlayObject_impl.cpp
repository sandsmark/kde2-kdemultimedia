...
void SplayPlayObject_impl::process_indata(DataPacket<mcopbyte> *inpacket) {
...
void SplayPlayObject_impl::processQueue() {
  ...
  currentPacket=(DataPacket<mcopbyte>*) packetQueue->peekqueue(0);
...
poTime SplayPlayObject_impl::currentTime() {
  poTime time;
  return time;
}
...
poTime SplayPlayObject_impl::overallTime() {
  poTime time;
  return time;
}

poCapabilities SplayPlayObject_impl::capabilities() {
  arts_debug("capabilities");
  return (poCapabilities)(capPause);
}
...
poState SplayPlayObject_impl::state() {
  return _state;
}
...
void SplayPlayObject_impl::seek(const class poTime& ) {
...
void SplayPlayObject_impl::getMoreSamples(int needLen) {
  ...
  if (file == NULL) {
    cout << "file NULL getMoreSamples"<<endl;
    exit(0);
  }
